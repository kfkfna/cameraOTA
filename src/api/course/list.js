...
export default {
  ...
  methods: {
    clickBtn(r) {
      this.$router.push({ path: '/course/cameraListDetail/' + r.cameraSn + '/' + r.deviceId})
    },
    resetData() {
    ...
    diffcameraOTA() {
      // console.log('diff cameraOTARepeatList: ' , this.cameraOTARepeatList);
      console.log('diff cameraOTALegalList: ' , this.cameraOTALegalList);
      // console.log('this.multipleSelection: ' , this.multipleSelection);
      // console.log('this.cameraSnList: ' , this.cameraSnList);
      // console.log('this.version: ' , this.version);
      // console.log('this.otaversion.modelId: ' , this.otaversion.modelId);
      if(this.otaversion.modelId === 4 || this.otaversion.modelId === 7) {
        // 请求
        console.log('modelId 4 or 7', this.otaversion)
        let postData = {
          "deviceIds": this.cameraOTALegalList,
      ...
      course.cameraOTA(this.version, this.cameraOTALegalList).then(response => {
        if (response.result) {
    ...
    async cameraOTA() {
      if (this.version === null) {
        ...
        this.cameraOTARepeatList = [];
        this.cameraOTALegalList = [];
        for (let i = 0; i < this.multipleSelection.length; i++) {
          const element = this.multipleSelection[i];
          if(element['fwVersion'] === this.version) {
            this.cameraOTARepeatList.push(element['cameraSn'])
          }else {
            this.cameraOTALegalList.push(element['deviceId'])
          }
        }
        console.log('cameraOTARepeatList: ' , this.cameraOTARepeatList, '', this.cameraOTARepeatList.length);
        console.log('cameraOTALegalList: ' , this.cameraOTALegalList, '', this.cameraOTALegalList.length);
        // {
        //   type: 'warning',
        //   message: `cameraSn: ${ cameraOTARepeatList.toString() } 这些原版本与将要升级的版本一致，不能进行OTA！`
        // }
        if(this.cameraOTALegalList.length < 1) {
          console.log('11');
          // `cameraSn: ${ this.cameraOTARepeatList.join('\n') } 这些原版本与将要升级的版本一致，不能进行OTA！`
          this.$confirm(`The device version is the same as the target version, and the execution fails.`, 'Tips', {
            type: 'error'
          })
          .then(_ => {
          })
        ...
        if(this.cameraOTARepeatList.length > 0 && this.cameraOTALegalList.length > 0) {
          console.log('22');
          // `cameraSn: ${ this.cameraOTARepeatList.join('\n') } 以上原版本与将要升级的版本一致，将会忽略。其他选择的camera,是否继续执行OTA？`
          await this.$confirm(`The some device version is the same as the target version, and the execution fails.`, 'Tips', {
            type: 'warning'
          })
          .then(d => {
            if(d === 'confirm') {
        ...
        if(this.cameraOTALegalList.length > 0 && this.cameraOTARepeatList.length < 1) {
          console.log('33');
          this.diffcameraOTA()
        }
