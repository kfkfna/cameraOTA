...
export default {
        ...
        return {
          modelName:"DoorBell",
            ...
            //课程列表
              ...
              cameraSnList:null,
              //查询条件集合
              cameraSn:this.$route.params.imei,
              action:null,
              courseList:null,
              cameraSnList1:null,
              // cameraOTAVO:{
    ...
    methods: {
      ...
      gotoCameraLogList(cameraSn){
        ...
        log.queryCameraLogList(this.pageNum,this.pageSize,this.cameraSn).then(response=>{
          if(response.data.data.length < 1){
            this.$message({
              type: 'error',
              message: 'There\'s no log on this camera'
            });
            return false;
          }else{
    ...
    handleClose(done) {
        ...
        queryCoursePageByCondition(pageNumUI = 1) {
            ...
            console.log(this.cameraSn)
            log.queryPanelLogList(this.pageNum, this.pageSize, this.cameraSn)
            .then(response=>{
                this.total=response.data.total;
